...
selected = option_menu(
    menu_title = None,
    options=["Home","Task","Execution","About"],
    icons=["house","card-list","diagram-3-fill","file-person"],
    default_index=0,
...
if selected == "About":
    ...
    st.header("Group Info :")
    col1,col2 = st.columns(2);

    with col1:
        st.subheader("PRN NO")
        st.write("2019BTECS00079")
        st.write("2019BTECS00080")
        st.write("2019BTECS00083")
        st.write("2019BTECS00084")

    with col2:
        st.subheader("NAME")
        st.write("Shantanu Joshi")
        st.write("Saurabh Nagare")
        st.write("Bhargav Kulkarni")
        st.write("Pravin Lokhande")    

if selected == "Home":
     st.image("mpImg.png" , width=700)

elif selected =="Task":
 st.subheader("Analytics for San Francisco Salaries DataUse:")
 st.write("DataUse the San Francisco Salaries Dataset to understand the income inequality in San Francisco city. Analyse the factors responsible for the promotions of certain employees. To better understand the distribution of the salaries, use different visualization libraries like scatter plots, box plots, whisker plots and the density plots.")
 ...
 if uploaded_file :
    ...
    vis = st.sidebar.selectbox(
      "Experiment",
      ("Select","Scatter plot","Box Plot","Density Plot",)
   )
    ...
    #scatter plots
    if vis == "Scatter plot":
     att1 = st.sidebar.selectbox("First attribute",df.columns)
     att2 = st.sidebar.selectbox("Second attribute",df.columns)
    ...
    dt = st.sidebar.button("Generate DT")
    if dt:
